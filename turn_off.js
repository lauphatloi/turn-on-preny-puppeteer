   ...
   console.log("Ready to click - Tích hợp nền tảng");
    ...
    try {
       ...
       // 2. Click Activate (The Zap Icon)
        ...
        const zapSelector = 'svg.lucide-zap-off';
        ...
        console.log("Clicked 'Hủy Kích hoạt trang' successfully!");

        // Wait 1 second for the popup modal to finish its opening animation
        await new Promise(resolve => setTimeout(resolve, 3000));

        // 3. Click Confirm
        console.log("Waiting for 'Hủy kích hoạt' button...");
        const confirmBtn = await page.waitForSelector('button[class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-5 [&_svg]:shrink-0 bg-primary text-primary-foreground shadow hover:bg-primary/90 h-9 px-4 py-2"]', { visible: true, timeout: 20000 });
        await confirmBtn.evaluate(btn => btn.click());
        
        console.log("Clicked 'Hủy kích hoạt'");
        ...
        await page.waitForSelector('button[class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-5 [&_svg]:shrink-0 bg-primary text-primary-foreground shadow hover:bg-primary/90 h-9 px-4 py-2"]', { hidden: true });
